...
/* Удаляем специальные случаи data-wide-emote — они больше не нужны */    
 ...
 // ─── UI render Panel picker (mini inline ) ──────────────────────────────────────────────
  ...
  function renderChatLine(line) {
      ...
      fragments.forEach(fragment => {
          ...
          const twitchImgClone = twitchEmoteDiv ? 
              ...
              null;
      // ← НОВОЕ: контекст предыдущего wrap-а для ZW через границу фрагментов
        const prevWrap = findLastSepWrapBefore(fragment);
  ...
  // ─── Ищем последний sep-emote-wrap перед данным фрагментом ──────────────────
  function findLastSepWrapBefore(fragment) {
      let el = fragment.previousElementSibling;
      while (el) {
          // Прямой sep-emote-wrap
          if (el.classList.contains('sep-emote-wrap')) return el;
          // Внутри text-fragment или ffz--inline
  ...
  function renderTextFragment(span, twitchImgClone = null, twitchEmoteDiv = null) {
      ...
      let lastNonZWWrap = findLastSepWrapBefore(span);
